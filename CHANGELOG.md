...
- **`applyCommand` releases the GIL for the two commands that carry geometry** — adding a link whose collision geometry is a mesh or a signed distance field spends seconds inside `Environment::applyCommand` building collision shapes, all of it holding the GIL, so every other Python thread stalled on what is pure C++ work. `applyCommand(AddLinkCommand)` and `applyCommand(AddSceneGraphCommand)` now drop the GIL around the C++ call, matching the `solve()` / `contactTest` guards, as do `getDiscreteContactManager` / `getContinuousContactManager` — `applyCommand` only updates a contact manager that is already cached, so with a cold cache the shape build lands in the first getter call instead. Python can still be re-entered from inside that region — an event callback, or the sampler of a lazy `createSignedDistanceField` field, which bullet evaluates per cell node while transcoding the shape — so `EventCallbackFn` now re-acquires the GIL (which it also needed for C++ threads such as the ROS 2 monitor; nanobind already handles the sampler). Known ceiling: a registered Python event callback plus a concurrent Python thread calling a non-releasing `Environment` method can deadlock, because upstream fires event callbacks under the environment's unique lock.

- **`nanobind <3` in the build requirements** — the pixi dependency was already capped, but `[build-system] requires` was not, so any build that does not go through the pixi env (a `TESSERACT_CPP_PREFIX` build against a system tesseract install, or a build-isolated `pip install .`) resolved nanobind 3.1.0 and failed to compile: 3.x changed the `nb::detail::ticket` constructor that the `NB_TRAMPOLINE` shims call.
- **`dependencies_ros.rosinstall` bumped to `7b8e51e`** — the rosutils bindings reference `tesseract_msgs::Geometry::SIGNED_DISTANCE_FIELD`, renamed upstream from `SDF_MESH` in `34669f2`; the previous pin predated it, so a ROS-sourced build failed to compile. Note `scripts/build_tesseract_ros2.sh` skips an existing `ws/src` checkout, so an already-cloned workspace needs its checkout moved by hand before rebuilding.

- **ROS 2 bindings** — two new modules wrapping [`tesseract_ros2`][tesseract_ros2], contributed by [@seonixx]. `pixi run install` builds them when a ROS 2 distro is sourced and skips them otherwise; published wheels stay ROS-free. Requires `python3-colcon-common-extensions`. Build design: `docs/developer/ros.md`:
